...
private:
    ...
    bool writeAllocate;         // write allocate policy
...
public:
	...
	int totalLatency = 0;
	int missCount = 0;
    	Cache(int setSize, int assoc, int blockSize, int hitLatency, int missLatency, bool writeAllocate, Cache* lowerCache);
	~Cache();
	...
	void missHandler(uint32_t setIndex, uint32_t tagIndex, int counterBlocksInLine);
	void printStats();
};
