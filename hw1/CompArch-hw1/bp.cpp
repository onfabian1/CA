...
public:
	...
	m_isGlobalTable(isGlobalTable), m_Shared(Shared), btbList(vector<btbRow*>(btbSize)), num_of_flush(0), num_of_update(0) {}
...
bool BTBTable::getTagBTB(uint32_t* btb_idx, uint32_t* tag_in_btb, uint32_t pc, uint32_t* predict_tag) {
	uint32_t btb_index = pc >> 2; // Same 
	uint32_t pc_tag = btb_index; // Eliminate closing zeros;
	int btb_index_size = index_size(m_btbSize);
	pc_tag = (pc_tag >> btb_index_size) & ((1 << m_tagSize) - 1);
	btb_index = btb_index & ((1 << btb_index_size) - 1);
	*btb_idx = btb_index;
	*predict_tag = pc_tag;
	if (btbTable[btb_index]) {
		*tag_in_btb = btbTable[btb_index]->get_tag();
		return true;
	}
	return false;
}
...
bool BP_predict(uint32_t pc, uint32_t *dst){
	...
	uint32_t* pc_tag = &temp_tag, btb_index = &temp_btb_index, tag_in_btb = &temp_tag_in_btb;
...
void BP_update(uint32_t pc, uint32_t targetPc, bool taken, uint32_t pred_dst){
	...
	bool flag = bp->getTagBTB(btb_index, tag_in_btb, pc, pc_tag);
	if ((*tag_in_btb == *pc_tag) && (flag)) { 
		bp->btbTable[*btb_index]->update_target(targetPc);
		bp->btbTable[*btb_index]->changem_fsmstate(taken, pc);
		bp->btbTable[*btb_index]->change_history(taken);
		if (((targetPc != pred_dst) && (taken)) || ( (pred_dst != (pc + 4) ) && (!taken) ) ) {
			bp->num_of_flush++;
		}
	} else { // / add to  the btb nes line .
		bp->btbTable[*btb_index] = new btbRow(*pc_tag, targetPc, isGlobalHist, isGlobalTable, Shared, fsmState, historySize);
		bp->btbTable[*btb_index]->change_fsmstate(taken, pc);
		bp->btbTable[*btb_index]->change_history(taken);
