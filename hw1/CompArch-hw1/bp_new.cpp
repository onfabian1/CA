...
class fsm_2bit {
	...
	public:
		...
		void next_state(unsigned curr_state_tmp);
};

int fsm_2bit::curr_state() {
	return curr_state;
}
...
void fsm_2bit::next_state(unsigned curr_state_tmp) {
...
bool BP_row::getFSMstate(uint32_t pc, uint32_t *dst) {
	...
	if (index >= 0 && index < (pow(2,history_size))) {
		if (global_fsms[index].curr_state() & 2) {
		    *(dst) = target; 
		    return true;
		} 
		else {
